...
**Acesso pela navegação**: a tela tem **botão próprio na barra inferior** (rótulo "Remédios", ícone de cápsula), posicionado entre Histórico e Agenda — a mesma posição que ocupa na sequência de swipe, de modo que a ordem da barra e a ordem do gesto coincidem. Continua também acessível por swipe, como todas as demais views.

*Histórico desta decisão*: as versões anteriores deste spec definiam o contrário — sem ícone na barra, acesso apenas por gesto — por receio de falta de espaço. Depois de implementada, ficou claro na prática que uma tela sem representação na navegação é difícil de encontrar e deixa a barra sem indicação de onde o usuário está. A medição posterior mostrou que o 6º item cabe: o problema real não era o número de colunas, e sim que o rótulo "Medicamentos" (59px) inchava a própria coluna e espremia as vizinhas. Resolvido com o rótulo curto "Remédios" (44px, mesma largura de "Calendário") e `grid-template-columns: repeat(6, minmax(0, 1fr))`, que trava as colunas em larguras iguais. Com o botão na barra, deixaram de existir a porta de entrada no cabeçalho do Histórico, o indicador de pager na view e o estado de contexto na nav — todos eram compensações para a ausência do botão.
...
- [Revista] Onde entra na navegação, já que a barra inferior está cheia (4 destinos + FAB)? **Decisão final: botão próprio na barra**, entre Histórico e Agenda, com rótulo curto "Remédios". A decisão original (só swipe, sem ícone) foi revertida após ver a tela em uso — sem representação na barra a funcionalidade fica escondida e o usuário perde a referência de onde está. A medição mostrou que o 6º item cabe em telas a partir de 320px desde que o rótulo seja curto e as colunas sejam travadas com `minmax(0, 1fr)`.
