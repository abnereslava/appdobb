...
**Acesso pela navegação**: em vez de ocupar um ícone fixo adicional na barra inferior, a tela de Medicamentos é alcançada por **gesto de arrastar/swipe** a partir da navegação principal — reaproveitando a navegação por gesto que o app já implementa entre as views. Fica definido que **não** haverá ícone fixo novo na nav (medição do CSS confirmou que um 6º item não caberia legível — ver `plan.md` §6.1), e que **deve existir indício visual** sinalizando que a tela existe, para não deixar a funcionalidade invisível para quem não descobrir o gesto por acaso.

Complementando o gesto, e sem custo de espaço na barra, a tela também tem uma **porta de entrada tocável no cabeçalho do Histórico de Saúde** (botão ao lado dos que já existem lá), garantindo um caminho acessível por toque para quem não usa gestos. Enquanto o usuário estiver na tela de Medicamentos, o item "Histórico" da nav fica num estado visual de contexto (mais fraco que o de aba ativa), para que a barra não fique completamente apagada.
