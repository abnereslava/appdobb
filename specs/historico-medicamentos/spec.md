...
Uma tela "Medicamentos" lista os medicamentos registrados do perfil ativo — cada um com nome, período de uso, dose/quantidade e regime de uso (contínuo ou por tempo determinado com posologia), observações, e um vínculo opcional a um evento do Histórico. Tem seu próprio botão de exportação em PDF, seguindo o mesmo padrão do `specs/gerador-pdf/`.

**Acesso pela navegação**: em vez de ocupar um 5º ícone fixo na barra inferior, a tela de Medicamentos é alcançada por **gesto de arrastar/swipe** a partir da navegação principal — reaproveitando a linguagem de gesto que o app já usa para trocar de perfil (swipe). O mecanismo exato (de qual tela o swipe parte, direção, indicador visual de que existe mais conteúdo a "puxar") fica para o `plan.md` — aqui fica definido que **não** será um ícone fixo adicional na nav.
...
2. Vê a lista de medicamentos do perfil, com destaque visual para os que estão "em uso" (contínuo sem data de fim, ou dentro do período de um tratamento por tempo determinado) versus os já encerrados.
3. Toca em "+ Novo Medicamento":
   ...
   - Regime de uso: **Contínuo** (sem data de fim definida) ou **Por tempo determinado** (com posologia: frequência — de quantas em quantas horas ou quantas vezes ao dia — e duração em dias; a data de fim é sugerida automaticamente a partir da data de início + duração, mas pode ser ajustada manualmente).
...
- Regime **Por tempo determinado**: tem frequência (de X em X horas, ou X vezes ao dia) e duração (em dias); a data de fim é calculada como `dataInicio + duracaoDias`, mas o usuário pode sobrescrevê-la manualmente a qualquer momento (edição ou "Encerrar uso hoje" antes do previsto, ex.: parou o tratamento antes do fim planejado).
- Um medicamento "em uso" é: contínuo sem data de fim, OU por tempo determinado cuja data de fim (calculada ou ajustada) ainda não passou.
...
- `regime` (`'continuo'` | `'temporario'`, obrigatório)
...
- `dataFim` (data, opcional — vazio em regime contínuo; sugerida automaticamente em regime temporário, editável)
...
- [ ] Tela "Medicamentos" com CRUD (criar, listar, editar, excluir) de medicamentos do perfil ativo, acessível por gesto de swipe (sem ícone fixo novo na nav inferior).
- [ ] Cada medicamento tem nome, dose/quantidade opcional, regime (contínuo ou por tempo determinado com posologia), data de início, data de fim (manual ou calculada), observações opcionais e evento relacionado opcional.
- [ ] Lista distingue visualmente medicamentos "em uso" dos encerrados, considerando os dois regimes.
- [ ] Regime "por tempo determinado" sugere a data de fim a partir da frequência/duração informadas, e permite ajuste manual.
- [ ] Botão "Encerrar uso hoje" no detalhe do medicamento, disponível nos dois regimes.
...
- [Respondida] Fonte/tamanho da lista estática de autocomplete? Nomes genéricos **e** comerciais, com o princípio ativo entre parênteses nos comerciais (ex.: "Tylenol (Paracetamol)"), o máximo de itens populares que for viável reunir. Além disso, o cadastro ganha campos estruturados de dose/quantidade (valor + unidade), não só texto livre.
...
- [Respondida] Botão "Encerrar uso hoje"? Sim, incluído — disponível nos dois regimes de uso.
